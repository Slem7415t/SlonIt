...
## ${\color{#DAA520}Слияние\ веток}$
Для слияния веток сначала переходим на ветку с которой будем объединять текущую ветку.
затем пишем команду `$ git merge имя_ветки_которую_вливаем` которая объединит ветки.        
`$ git branch -f master ORIG_HEAD` - позволяет откатить слияние с веткой ***master***.

## ${\color{#DAA520}Удаление\ веток}$
После слияния веток ненужную ветку можно удалить командой `$ git branch -d имя_ветки`, удаление не объединенной ветки вызовет ошибку.       
`$ git branch -D имя_ветки` - удалит не объединенную ветку, но можно потерять ее ***commit***ы если их оставить без ветки на долго.     
`$ git branch имя_удаленной_ветки индификатор_commit` - вернет ***commit***ам их ветку.

## ${\color{#DAA520}Reference\ log}$
\- логи(история изменений веток).

`$ git reflog имя_ветки` - выведет историю изменений ветки.     
`$ git reflog` - выведет историю изменений ***HEAD***.      
Ключ ***--date=iso*** добавит к выведенным логам даты.

`$ git log` - выведет логи ***commit***.

`$ git blame` - выведет кто написал, какую строку.

## ${\color{#DAA520}Удаление\ лишних\ файлов}$
`$ git clean -dxf` - удалит все не отслеживаемые файлы, директории(***d***), игнорируемые файлы(***x***) из рабочей директории.

## ${\color{#DAA520}Отмена\ изменений}$
`$ git reset индификатор_commit` - отменит изменения и вернет нас к указанному ***commit***.        
флаг ***--hard*** - для жесткого сброса.        
флаг ***--soft*** - для мягкого сброса, он оставляет ***commit*** и в ***рабочей директории*** и в ***index***.     
флаг ***--mixed*** - для гибридного сброса, он оставляет ***commit*** только в ***рабочей директории***(используется по умолчанию, когда не указан флаг).

## ${\color{#DAA520}Сравнение}$
`$ git diff индификатор_commit_или_имя_ветки индификатор_commit_или_имя_ветки` - выводит информацию об изменениях в ветках или ***commit***.        
`$ git diff` - выводит информацию об изменении между ***рабочей директорией*** и ***index***(игнорирует не отслеживаемые файлы).      
`$ git diff HEAD` - выводит информацию об изменении между ***рабочей директорией*** и ***Repository***(игнорирует не отслеживаемые файлы).      
`$ git diff --cached` - выводит информацию об изменении между ***index*** и ***Repository***.
